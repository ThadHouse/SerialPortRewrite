...
AHRS::AHRS(I2C::Port i2c_port_id, uint8_t update_rate_hz) {
     ...
     * @param update_rate_hz Custom Update Rate (Hz)
     */
AHRS::AHRS(frc2::SerialPort::Port serial_port_id, AHRS::SerialDataType data_type, uint8_t update_rate_hz) {
 ...
 * @param serial_port_id SerialPort to use
 */
AHRS::AHRS(frc2::SerialPort::Port serial_port_id) {
...
void AHRS::SerialInit(frc2::SerialPort::Port serial_port_id, AHRS::SerialDataType data_type, uint8_t update_rate_hz) {
    std::cout << "Serial Port Init! " << std::endl;
    printf("Instantiating navX-Sensor on Serial Port %d.\n", serial_port_id);
    commonInit(update_rate_hz);
    #ifdef __linux__
    if (IsRaspbian() && (serial_port_id == frc2::SerialPort::Port::kMXP)) {
        io = new RegisterIOMau(update_rate_hz, ahrs_internal, ahrs_internal);
    } else {
