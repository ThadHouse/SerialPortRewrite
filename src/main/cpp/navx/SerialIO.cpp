...
SerialIO::SerialIO( frc2::SerialPort::Port port_id,
                    uint8_t update_rate_hz,
                    bool processed_data,
                    IIOCompleteNotification *notify_sink,
                    IBoardCapabilities *board_capabilities ) {
    this->serial_port_id = port_id;
    is_usb = ((port_id != frc2::SerialPort::Port::kMXP) &&
    		  (port_id != frc2::SerialPort::Port::kOnboard));
    ypr_update_data = {};
    gyro_update_data = {};
    ahrs_update_data = {};
    ahrspos_update_data = {};
    ...
    std::cout << "Serial Port Created! " << (intptr_t)serial_port << std::endl;
...
frc2::SerialPort *SerialIO::ResetSerialPort()
{
    if (serial_port != 0) {
...
frc2::SerialPort *SerialIO::GetMaybeCreateSerialPort()
{
    if (serial_port == 0) {
        try {
        	printf("Opening %s serial port to communicate with navX-MXP/Micro.\n", (is_usb ? "USB " : "TTL UART "));
            serial_port = new frc2::SerialPort(57600, serial_port_id);
...
void SerialIO::Run() {
    ...
    try {
        serial_port->Reset();
        std::cout << "Writing to NAVX " << cmd_packet_length << std::endl;
        auto written = serial_port->Write( stream_command, cmd_packet_length );
        ...
        std::cout << "Writing to NAVX " << cmd_packet_length << std::endl;
        written = serial_port->Write( stream_command, cmd_packet_length );
        std::cout << "Written " << written << std::endl;
        serial_port->Flush();
        port_reset_count++;
    ...
    while (!stop) {
        try {
            ...
            if ( signal_transmit_integration_control ) {
                std::cout << "Transmit signal control " << std::endl;
                ...
                while (i < bytes_read) {
                    ...
                    if ( received_data[i] != PACKET_START_CHAR ) {
                        ...
                        discarded_bytes_count++;
                            ...
                            //SmartDashboard::PutRaw("Raw", received_data);
                        #endif
                        continue;
                    } else {
